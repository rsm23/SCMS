...
@section('content')
        ...
        <div class="row">
            <article class="card blog-body">
                <div class="card-body">
                    ...
                    <h6 class="card-subtitle mb-2 text-muted">{{ $post->created_at->diffForHumans() }}{!!  ($edited) ? '<span class="text-success float-right"> Edited by : '.$edited->name.'</span>' : ''  !!}</h6>
                    @markdown($post->body)
                </div>
            </article>
        </div>
    ...
    <div id="addReply" class="d-flex align-content-center mt-5 p-2">
        <div class="container">
            @if (auth()->check())
                <div class="row mt-5">
                    <form action="{{ route('blogAddReply', [$post->category, $post->slug]) }}" method="POST"
                          style="width: 100%">
                        {{ csrf_field() }}
